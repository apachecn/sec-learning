# 内网渗透之获取 windows 远程桌面（RDP）连接记录

> 原文：[http://book.iwonder.run/内网渗透/内网渗透/内网渗透之获取 windows 远程桌面（RDP）连接记录/1.html](http://book.iwonder.run/内网渗透/内网渗透/内网渗透之获取 windows 远程桌面（RDP）连接记录/1.html)

利用条件：就是 mstsc 连接的时候，管理员勾选了自动保存密码连接的选项。

目标 ip：172.xx.x.1 被控制 ip:172.xx.x.12
...
如此可见，此服务器保存了两个连接地址，其中的 172.xx.x.1 就是我们想要获取到的地址。
...
![image](img/3092b1c6b08319a10e73bdeb25e45907.png)
...
![image](img/7789c08fe50fe1529907fbf3e26ceb2e.png)
...
找到其关联的 MasterKey，这个 MasterKey 就是加密凭据的密钥，即解密 pbData 所必须的东西。
...
![image](img/37319da998b2c510186b7adad86139b5.png)

![image](img/fc22289bca2e63952a624200ea21598f.png)
