...
> 原文：[http://book.iwonder.run/iot/Amazon Kindle Fire HD/CVE-2018-11021.html](http://book.iwonder.run/iot/Amazon Kindle Fire HD/CVE-2018-11021.html)

## 一、漏洞简介

Amazon Kindle Fire HD（3rd）Fire OS 4.5.5.3 内核组件中的内核模块/omap/drivers/video/omap2/dsscomp/device.c 允许攻击者通过设备/ dev 上 ioctl 的参数注入特制参数/ dsscomp 与命令**1118064517**并导致内核崩溃。

要探索此漏洞，必须打开设备文件/ dev / dsscomp，并使用命令**1118064517**和精心设计的有效负载作为第三个参数在此设备文件上调用 ioctl 系统调用。
