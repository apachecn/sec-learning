# sqlmap bypass d 盾 tamper

> 原文：[http://book.iwonder.run/Tools/Sqlmap tamper/1.html](http://book.iwonder.run/Tools/Sqlmap tamper/1.html)
...
class BypassDog(object):
    ...
    def djc(self):
        for u in self.ybs:
            ...
            time.sleep(0.01) #0.01 CPU 低于 50% 0.005CPU 低于 70 0.003CPU 低于 95，根据内容数量来手动设置是否需要延时
            self.djcs.append(u)
            self.calc+=1
